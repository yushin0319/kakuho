  ...
  pattern2: [
    {
      ...
      ticketTypes: [{ id: 3, seat_group_id: 2, type_name: 'VIP', price: 8000 }],
    },
  ],
};
...
describe('SeatGroupSelector', () => {
  ...
  it('選択ボタンをクリックするとonSelectが呼ばれる', async () => {
    ...
    render(<SeatGroupSelector {...defaultProps} onSelect={onSelect} />, {
      wrapper: TestWrapper,
    });
  ...
  it('キャンセルボタンをクリックするとonCloseが呼ばれる', async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    render(<SeatGroupSelector {...defaultProps} onClose={onClose} />, {
      wrapper: TestWrapper,
    });
  ...
  it('open=falseの場合ダイアログが表示されない', () => {
    render(<SeatGroupSelector {...defaultProps} open={false} />, {
      wrapper: TestWrapper,
    });
  ...
  it('空のseatDictではパターンが表示されない', () => {
    render(<SeatGroupSelector {...defaultProps} seatDict={{}} />, {
      wrapper: TestWrapper,
    });
    expect(screen.getByText('チケット情報選択')).toBeInTheDocument();
